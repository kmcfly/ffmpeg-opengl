...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
        ...
        height = is->height;
        width = ((int)lrintf(height * aspect_ratio)) & -3; // 20090120
        if (width > is->width) {
            width = is->width;
            height = ((int)lrintf(width / aspect_ratio)) & -3; // 20090120
        }
        x = (is->width - width) / 2;
    ...
    for(;;) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            if (cur_stream) {
                int ns, hh, mm, ss;
                int tns, thh, tmm, tss;
                tns = cur_stream->ic->duration/((int64_t)1000000);
                thh = tns/3600;
                tmm = (tns%3600)/60;
                tss = (tns%60);
